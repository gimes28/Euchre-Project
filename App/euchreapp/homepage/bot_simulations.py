...
class MonteCarloSimulation():
    ...
    def monte_carlo_card_evaluator(self, original_state, player_hand, play_order, trump_suit, trump_maker, going_alone, team_tricks, num_simulations=50):
        ...
        # Create a deck of all possible (excluding the player's hand and the previous cards and the cards played in the current trick)
        suits = ["hearts", "diamonds", "clubs", "spades"]
        ranks = ["A", "K", "Q", "J", "10", "9"]
        ...
        played_cards = copy.deepcopy(original_state["played_cards"])

        player_position = len(played_cards)

        # Remove the player's hand and the previous cards
        known_cards = []
        for card in player_hand:
            known_cards.append(card)
        for trick in sim_previous_tricks.values():
            for card in trick:
                known_cards.append(card.card)
        for card in played_cards:
            known_cards.append(card.card)

        unknown_cards = [card for card in all_cards if card not in known_cards]
        
        card_probabilities = {str(card): 0 for card in player_hand}

        # Get only valid cards to play
        if played_cards:
            if played_cards[0].card.is_left_bower(trump_suit):
                lead_suit = played_cards[0].card.next_suit()
            else:
            ...
            if lead_suit == trump_suit:
                valid_cards = [card for card in player_hand if card.suit == trump_suit or card.is_left_bower(trump_suit)]
            else:
                valid_cards = [card for card in player_hand if card.suit == lead_suit and not card.is_left_bower(trump_suit)]
        else:
        ...
        for card in valid_cards:
            ...
            for _ in range(num_simulations):
                ...
                # Instead of deep copying the players, create a mapping of names to new Bot objects
                sim_players = []
                name_to_bot = {}
                for p in play_order:
                    new_bot = Bot(name=p.name, partner=p.partner, team=p.team)
                    sim_players.append(new_bot)
                    name_to_bot[p.name] = new_bot
                ...
                trick = copy.deepcopy(played_cards)

                trick.append(PlayedCard(card=card, player=[p for p in play_order if p.name == "Player"][0]))
                ...
                for i in range(player_position + 1, 4):
                    bot = sim_players[i]
                    if bot.name == "Player":
                        continue
                ...
                winner = name_to_bot[winner.name]
                ...
                winner_idx = sim_players.index(winner)
                sim_players = sim_players[winner_idx:] + sim_players[:winner_idx]
                ...
                for _ in range(4):
                    ...
                    if trick:
                        ...
                        # Rotate the players list so the winner leads the next trick
                        winner_idx = sim_players.index(winner)
                        sim_players = sim_players[winner_idx:] + sim_players[:winner_idx]
